...
static __attribute__((inline)) void CLCD_ToggleEnable(void) {
    LATCbits.LATC6 = 1;
...
static void CLCD_SendCmd(uint8_t cmd) {
    ...
    uint8_t port = PORTD & 0x0F;
    LATD = ((cmd & 0xF0) | port);
    CLCD_ToggleEnable();
    _delay((unsigned long)((1)*(16000000/4000000.0)));
    LATD = (((cmd<<4) & 0xF0) | port);
    CLCD_ToggleEnable();
...
# 68 "xlcd.c"
    ...
    CLCD_SendCmd(0x20 | 0x00 | 0x08);


    CLCD_SendCmd(0x01);
    CLCD_SendCmd(0x08 | 0x04);
    CLCD_SendCmd(0x04 | 0x02);
    CLCD_SendCmd(0x80);
}
...
void CLCD_SetPos(uint8_t row, uint8_t col) {
    if (row) {
        CLCD_SendCmd(0x80 | (0x40 + col));
    } else {
        CLCD_SendCmd(0x80 | (0x00 + col));
    }
}
